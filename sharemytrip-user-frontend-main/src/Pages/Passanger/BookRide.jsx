    ...
    try {
      ...
      if (validate()) {
        ...
        const response = await axios.get(
          `http://localhost:8095/user/passengers/rides/filter`,
          {
            params: {
              fromLocation,
              toLocation,
  ...
  const handleBookRide = async () => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:8095/user/passengers/${auth.id}/book`,
        {
          passengerId: auth.id,
      ...
      if (response.status === 200) {
        await axios.post(
          `http://localhost:8095/email/send-passenger-booked-confirmation?passengerEmail=${encodeURIComponent(
            auth.email
          )}`
        );
