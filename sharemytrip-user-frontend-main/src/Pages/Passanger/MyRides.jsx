...
const MyRides = () => {
  ...
  const onCancel = async () => {
    try {
      const response = await axios.delete(
        `http://localhost:8095/user/passengers/cancel/${selectedRide.passengerRideId}`
      );
      if (response.status === 204) {
        sendPublisherEmail(selectedRide.publisherEmail);
        await axios.post(
          `http://localhost:8095/email/send-passenger-canceled-notification?passengerEmail=${encodeURIComponent(
            auth.email
          )}`
        );
  ...
  const onPayNow = async (ride) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:8095/user/passengers/pay",
        payload
      );
  ...
  const fetchRides = async () => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:8095/user/passengers/${auth.id}/rides`
      );
      const ridesData = response.data;
      
      // Fetch publisher details for each ride
      const publisherRequests = ridesData.map((ride) =>
        axios.get(`http://localhost:8095/user/publishers/${ride.publisherId}`)
      );
  ...
  const sendPublisherEmail = async(publisherEmail) =>{
    try {
      axios.post(`http://localhost:8095/email/send-pasenger-canceled-notification-publisher?publisherEmail=${encodeURIComponent(
        publisherEmail)}`)
    } catch (error) {
      console.log(error);
    }
