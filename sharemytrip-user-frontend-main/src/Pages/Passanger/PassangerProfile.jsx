...
const PassengerProfile = () => {
  ...
  const fetchProfileData = async (passengerId) => {
    try {
      const response = await axios.get(`http://localhost:8095/user/passengers/${passengerId}`);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.put(`http://localhost:8095/user/passengers/${auth.id}`, updatedProfile);
