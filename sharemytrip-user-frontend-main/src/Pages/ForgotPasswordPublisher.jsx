...
const ForgotPasswordPublisher = () => {
  ...
  const handleSendOtp = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post(`http://localhost:8095/user/publishers/reset-password/${email}`);
  ...
  const handleVerifyOtp = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post(`http://localhost:8095/user/publishers/verify-otp/${email}/${otp}`, null, {
        params: {
          otp: otp,
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      const response = await axios.put(`http://localhost:8095/user/publishers/update-password`, null, {
        params: {
          email: email,
          newPassword: newPassword
        },
        ...
        <div className="bg-white rounded-lg shadow-md w-full max-w-4xl flex overflow-hidden">
          ...
          <div className="w-full md:w-1/2 p-8">
            ...
            <p className="text-center mt-4">
              ...
              <NavLink to="/log-in/publisher" className="text-red-600 hover:text-red-700">
                Login here
              </NavLink>
            </p>
          </div>
            ...
            <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
              ...
              <div className="p-4 md:p-5">
                <form onSubmit={handleResetPassword} className="space-y-4">
                  ...
                  <button
                    type="submit"
                    className="w-full text-white bg-[#ff6f61] focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center"
                  >
                    Reset Password
                  </button>
                </form>
