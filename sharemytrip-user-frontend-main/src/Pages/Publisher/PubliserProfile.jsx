...
const PublisherProfile = () => {
  ...
  const fetchProfileData = async (publisherId) => {
    try {
      const response = await axios.get(`http://localhost:8095/user/publishers/${publisherId}`);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.put(`http://localhost:8095/user/publishers/${auth.id}`, updatedProfile);
