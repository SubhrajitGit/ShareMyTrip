...
const CreateRide = () => {
      ...
      if (from_location && to_location) {
        try {
          const distanceResponse = await axios.post(
            "http://localhost:8095/fare/distance",
            {
          ...
          if (distanceResponse.status === 200) {
            ...
            const fareResponse = await axios.post(
              "http://localhost:8095/fare/calculate",
              {
                fromLocation: from_location,
                toLocation: to_location,
              }
            ...
            if (fareResponse.status === 200) {
              ...
              const JourneyResponse = await axios.post(
                "http://localhost:8095/fare/calculateJourneyTime",
                {
                  fromLocation: from_location,
                  toLocation: to_location,
                }
  ...
  useEffect(() => {
    const updateMinTime = () => {
      const now = new Date();
      const selectedDate = new Date(date_of_journey);
  ...
  const handleConfirm = async () => {
    try {
      ...
      const response = await axios.post(
        `http://localhost:8095/user/publishers/${auth.id}/rides`,
        {
          fromLocation: from_location,
          toLocation: to_location,
          distance: distance,
      ...
      if (response.status === 200) {
        await axios.post(
          `http://localhost:8095/email/send-publisher-confirmation?publisherEmail=${encodeURIComponent(
            auth.email
          )}`
        );
