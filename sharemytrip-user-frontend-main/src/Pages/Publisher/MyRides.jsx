...
const MyRides = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:8095/user/publishers/${auth.id}/rides`)
      .then((response) => {
  ...
  const sendEmail = async (passengerEmail) => {
    try {
      await axios.post(
        `http://localhost:8095/email/send-publisher-canceled-notification-passenger?passengerEmail=${encodeURIComponent(
          passengerEmail
        )}`
      );
    } catch (error) {
  ...
  const handleViewPassengers = (rideId) => {
    ...
    if (!showPassengers) {
      axios
        .get(`http://localhost:8095/user/publishers/${rideId}/passengers`)
  ...
  const handleStartRide = (rideId) => {
    axios
      .put(`http://localhost:8095/user/publishers/${rideId}/start`)
      .then(() => {
        setRides((prevRides) =>
          prevRides.map((ride) =>
  ...
  const handleEndRide = (rideId) => {
    axios
      .put(`http://localhost:8095/user/publishers/${rideId}/end`)
      .then(() => {
        setRides((prevRides) =>
          prevRides.map((ride) =>
            ride.publisherRideId === rideId
  ...
  const handleCancelRide = async (rideId) => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:8095/user/publishers/${rideId}/passengers`
      );
      const passengersData = response.data;

      // Send email to publisher
      await axios.post(
        `http://localhost:8095/email/send-publisher-canceled-notification?publisherEmail=${encodeURIComponent(
          auth.email
        )}`
      );
      ...
      // Delete the ride
      await axios.put(`http://localhost:8095/rides/cancel-published/${rideId}`);
