...
const Payment = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("http://localhost:8095/rides/completed");
  ...
  const handleViewDetails = async (ride) => {
    try {
      const responseDetailsRide = await axios.get(`http://localhost:8095/user/publishers/${ride.publisherRideId}/passengers`);
